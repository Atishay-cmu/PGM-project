...
class ActionModel(nn.Module):
    def __init__(self, config): #action size, in dim, 
        ...
        if(config.OPP):
          #config.OBS_EMBED = 16
          self.action_encoder = Encoder(in_dim=config.ACTION_SIZE*config.NUM_AGENTS, hidden=config.HIDDEN, embed=config.ACTION_EMBED)
        else:
    ...
    def decode(self, latent_action, latent_state):

        if(self.config.A_USE_RNN):
          action_feat = F.relu(self.action_decoder1(latent_state.get_features()))
        else:
