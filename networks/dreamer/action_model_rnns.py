...
class NormalLatentDist(nn.Module):
    def __init__(self, in_dim, n_latents, hidden_size):
        super().__init__()
    ...
    def forward(self, x):
        ...
        latents = mean + torch.randn_like(std) * std
...
class RSSMTransition(nn.Module):
    ...
    def forward(self, obs, prev_states, mask=None):
        ...
        if(self.config.A_DISCRETE_LATENTS):
          ...
          rssm_state = RSSMStateDiscrete(logits=logits, stoch=stoch_state, deter=deter_state)
        else:
...
class RSSMRepresentation(nn.Module):
    ...
    def forward(self, action_embed, obs, prev_states, mask=None):
        ...
        if(self.config.A_DISCRETE_LATENTS):
          ...
          posterior_states = RSSMStateDiscrete(logits=logits, stoch=stoch_state, deter=prior_states.deter)
        else:
